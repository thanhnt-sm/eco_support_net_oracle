...
| Surface | Evidence |
|---|---|
| CLI commands: `validate`, `preflight`, `baseline`, `snapshot refresh/show/diff`, `init`, `config show/validate`, `oracle-check`, `migrate`, `version` | `src/DataGuard.Cli/Program.cs`: `validateCommand`, `preflightCommand`, `baselineCommand`, `snapshotCommand` (`snapshotRefreshCommand`, `snapshotShowCommand`, `snapshotDiffCommand`), `initCommand`, `configCommand` (`configShowCommand`, `configValidateCommand`), `oracleCheckCommand`, `migrateCommand`, `versionCommand`; `OfflineManifestWriter` writes the bounded operator-owned artifact. |
