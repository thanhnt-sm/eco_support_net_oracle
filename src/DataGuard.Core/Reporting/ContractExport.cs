...
public static class ContractExportWriter
{
            ...
            switch (contract)
            {
                case EntityDescriptor entity:
                    export.Entities.Add(new EntityExport
                    {
                        ...
                        Properties = entity.Properties
                            ...
                            .ThenBy(property => property.ColumnName, StringComparer.Ordinal)
                            .ToList(),
                    });
                    break;
                ...
                case StoredProcedureDescriptor procedure:
                    export.StoredProcedures.Add(new StoredProcedureExport
                    {
                        ...
                        Parameters = procedure.Parameters
                            ...
                            .OrderBy(parameter => parameter.OrdinalPosition)
                            .ThenBy(parameter => parameter.Name, StringComparer.Ordinal)
                            .ToList(),
                        ResultColumns = procedure.ResultColumns.Select(ToColumnExport).OrderBy(column => column.Name, StringComparer.Ordinal).ToList(),
                    });
                    break;
                ...
                case DatabaseSchemaDescriptor schema:
                    export.Tables.AddRange(schema.Tables
                        ...
                        {
                            Name = table.Name,
                            Columns = table.Columns.Select(ToColumnExport).OrderBy(column => column.Name, StringComparer.Ordinal).ToList(),
                        }));
                    break;
            }
        }
    ...
    /// <summary>Writes the same deterministic contract schema as YAML.</summary>
    public static async Task WriteYamlAsync(string outputPath, string provider, IEnumerable<ContractDescriptor> contracts, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var yaml = YamlSerializer.Serialize(Build(provider, contracts));
        await WriteAtomicallyAsync(outputPath, yaml, cancellationToken).ConfigureAwait(false);
    }

    internal static async Task WriteAtomicallyAsync(string outputPath, string content, CancellationToken cancellationToken)
    {
...
public static class TypeScriptContractWriter
{
    ...
    public static async Task WriteAsync(string outputPath, IEnumerable<EntityDescriptor> entities, CancellationToken cancellationToken = default)
    {
        ...
        var usedTypeNames = new Dictionary<string, int>(StringComparer.Ordinal);
        foreach (var entity in entities.OrderBy(entity => entity.Name, StringComparer.Ordinal))
        {
            var typeName = AllocateIdentifier(entity.Name, usedTypeNames);
            builder.AppendLine($"export interface {typeName} {{");
            var usedPropertyNames = new Dictionary<string, int>(StringComparer.Ordinal);
            foreach (var property in entity.Properties.OrderBy(property => property.Name, StringComparer.Ordinal).ThenBy(property => property.ColumnName, StringComparer.Ordinal))
            {
                var tsType = ToTypeScriptType(property.ClrTypeName);
                var optional = property.IsNullable ? "?" : string.Empty;
                var propertyName = AllocatePropertyName(property.Name, usedPropertyNames);
                builder.AppendLine($"  {JsonSerializer.Serialize(propertyName)}{optional}: {tsType};");
            }
        ...
        await ContractExportWriter.WriteAtomicallyAsync(outputPath, builder.ToString(), cancellationToken).ConfigureAwait(false);
    }

    private static string ToTypeScriptType(string clrTypeName) =>
        TypeMap.TryGetValue(clrTypeName, out var mapped) ? mapped : "unknown";

    private static string AllocateIdentifier(string name, IDictionary<string, int> allocations)
    {
        var normalized = string.IsNullOrEmpty(name) ? "Contract" : new string(name.Select((character, index) =>
            (character is >= 'A' and <= 'Z') || (character is >= 'a' and <= 'z') || character == '_' || (index > 0 && character is >= '0' and <= '9')
                ? character : '_').ToArray());
        if (char.IsDigit(normalized[0]))
        {
            normalized = $"_{normalized}";
        }
        return Allocate(normalized, allocations);
    }

    private static string AllocatePropertyName(string name, IDictionary<string, int> allocations) =>
        Allocate(string.IsNullOrEmpty(name) ? "property" : name, allocations);

    private static string Allocate(string name, IDictionary<string, int> allocations)
    {
        if (!allocations.TryGetValue(name, out var count))
        {
            allocations[name] = 1;
            return name;
        }
