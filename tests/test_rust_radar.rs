...
fn test_eci_critical_fragility_calculation() {
    ...
    assert!(matches!(candidate.risk_tier, RiskTier::CriticalEmergency | RiskTier::HighUrgency));
...
fn test_eci_stable_repo_calculation() {
    ...
    assert!(matches!(candidate.risk_tier, RiskTier::Stable | RiskTier::Moderate));
}
